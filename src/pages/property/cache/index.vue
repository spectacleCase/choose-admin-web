  ...
  <div class="p-6 bg-gray-100 performance-monitor">
    ...
    <div class="grid grid-cols-1 gap-6 mb-10 lg:grid-cols-2">
      ...
      <section class="p-6 bg-white rounded-lg shadow-md min-h-[300px]">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <InfoCard title="用户使用率" :value="`${data.cpu.used}%`" :threshold="90" />
          <InfoCard title="系统使用率" :value="`${data.cpu.sys}%`" :threshold="90" />
      ...
      <section class="p-6 bg-white rounded-lg shadow-md min-h-[300px]">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <InfoCard title="使用率" :value="`${data.mem.usage}%`" :threshold="90" />
        </div>
      </section>
    </div>
    ...
    <section class="p-6 mb-10 bg-white rounded-lg shadow-md">
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div v-for="disk in data.sysFiles" :key="disk.dirName" class="p-4 bg-gray-50 rounded-lg shadow min-h-[250px]">
          <h3 class="mb-4 text-lg font-semibold text-gray-600">{{ disk.typeName }}</h3>
          <div class="grid grid-cols-2 gap-4">
            ...
            <InfoCard title="已用百分比" :value="`${disk.usage}%`" :threshold="90" />
          </div>
        </div>
      </div>
    </section>
